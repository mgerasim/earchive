...
class AttributesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @attribute.save
        format.html { redirect_to @attribute, notice: 'Аттрибут успешно сохранен.' }
  ...
  def update
    ...
    respond_to do |format|
      if @attribute.update_attributes(params[:attribute])
        format.html { redirect_to @attribute, notice: 'Аттрибут успешно обновлен.' }
        format.json { head :ok }
      else
